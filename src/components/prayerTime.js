...
class PrayerTime extends React.Component {
  ...
  async getPrayerTime() {
    ...
    }T00:00:00.000%2B01:00`;
    const url =
      "https://api.masjidbox.com/1.0/masjidbox/landing/athany?get=wg&days=1&begin=";
    const headers = {
    ...
    await fetch(url + prayerTime, {
      ...
      .then((data) =>
        this.setState({
          prayerTimeTable: this.prayerTimeTableFormatter(data.timetable[0]),
        })
      )
      .catch((error) => {
    ...
    return (
      <div className="prayer-cards-group">
        {prayerTimeTable.map((info, i) => (
          <this.prayerCard prayerInfo={info}></this.prayerCard>
        ))}
      </div>
    );
  }
